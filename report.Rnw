...
df$metric <- sapply(df$metric, simpleCap)
@
...
Green bars indicate significant activity for a given metric. This badge is added only when a metric is above the 75th percentile and exceed a minimum frequency. These include highly discussed (for tweets), highly cited (for forks), and highly recommended (for stars).  This ImpactStory report was generated using \href{http://ropensci.org}{rOpenSci's} \href{http://ropensci.github.com/rImpactStory/}{rImpactStory package}. The R script and template necessary to reproduce this PDF are available \href{https://github.com/ropensci/ImpactReport}{here}. A more detailed report can be viewed at \url{http://impactstory.org/collection/d4npn7}.
\end{document}
